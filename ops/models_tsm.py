...
class VideoNet(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                ...
                temporal_pool=False, element_filter=False, cdiv=2, target_transforms=None):
    ...
    def _prepare_base_model(self, backbone):
        if 'resnet' in backbone:
            ...
            # import nets.C2D_ECals as C2D_ECal
            base_model_name = 'nets.TSM_' + self.net
            print('=> base model: {}, with backbone: {}'.format(base_model_name, backbone))
            TSM_EF = import_module(base_model_name)
            # block = 'EF_%s'%self.net
            # print(block)
            self.base_model = getattr(TSM_EF, backbone)(pretrained=True if self.pretrain == 'imagenet' else False,
                cdiv=self.cdiv, n_segment=self.num_segments)
