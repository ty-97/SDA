...
class VideoNet(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                ...
                dropout=0.5, partial_bn=False, print_spec=True, pretrain='imagenet', ef_lr5=False,
                fc_lr5=False, element_filter=False, cdiv=4, target_transforms=None):
    ...
    def _prepare_base_model(self, backbone):
        ...
        if 'resnet' in backbone:
            if self.element_filter:
                # import nets_test.C2D_ECals as C2D_ECal
                base_model_name = 'nets_test.C2D_' + self.net
                print('=> base model: {}, with backbone: {}'.format(base_model_name, backbone))
                C2D_EF = import_module(base_model_name)
                # block = 'EF_%s'%self.net
                # print(block)
                self.base_model = getattr(C2D_EF, backbone)(pretrained=True if self.pretrain == 'imagenet' else False,
                    cdiv=self.cdiv, num_segments=self.num_segments)
            else:
                print('=> base model: C2D, with backbone: {}'.format(backbone))
                import nets_test.C2D as C2D
                self.base_model = getattr(C2D, backbone)(pretrained=True if self.pretrain == 'imagenet' else False)
            #
    ...
    def get_optim_policies(self):
        ...
        for name, m in self.named_modules():
            if 'eft' in name:
                ...
                # elif 'qVec' in name:
                #     ef_weight.append(list(m.parameters()))
        ...
        return [
            ...
            {'params': custom_ops, 'lr_mult': 1, 'decay_mult': 1,
             'name': "custom_ops"},
            # for ef
            {'params': ef_weight, 'lr_mult': 1, 'decay_mult': 1,
             'name': "ef_weight"},
    ...
    def forward(self, input):
        ...
        base_out, weights = self.base_model(input)
        if self.dropout > 0:
