...
from nets.SDA import Ensemble_fcstlf as Ensemble
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, use_ef=False, cdiv=2, num_segments=8):
        ...
        if self.use_ef:
            print('=> Using Ensemble_avg with cdiv: {}'.format(cdiv))
            self.eft = Ensemble(planes, planes, cdiv, num_segments)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, cdiv=2, n_seg=8):
        ...
        if blocks >= 23:
            n_round = 2
            print('=> Using n_round {} to insert ATE'.format(n_round))
        #
...
if __name__ == "__main__":
    ...
    net = resnet50(False, num_classes=174, cdiv=8, num_segments=8)
    net.eval()
    output = net(inputs)
    print(output.size())
