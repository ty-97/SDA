...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, use_ef=False, cdiv=4, n_segment=8, fold_div=8, place=None):
        ...
        if self.use_ef:
            ...
            self.eft = Ensemble(planes, planes, cdiv, n_segment)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, cdiv=4, n_seg=8, fold_d=8, pla=None):
        ...
        for i in range(1, blocks):
            if i % n_round == 0:
                layers.append(block(self.inplanes, planes, n_segment=n_seg, fold_div=fold_d, place=pla, cdiv=cdiv, use_ef = True))
            else:
